...
from services.menu import (
    listar_menu_publico,
    listar_menu_publico_por_categoria,
    obtener_plato_id,
    listar_categorias
)
...
def get_menu():
    ...
    if categoria:
        ...
        menu = listar_menu_publico_por_categoria(categoria)

        if not menu:
            return jsonify({
                "message": "No hay ningún plato en esa categoria"
            }), 404

        return jsonify(menu), 200

    menu = listar_menu_publico()
...
def get_plato(plato_id):
    ...
    if not plato:
        return jsonify({
            "message": "El plato no pudo ser encontrado"
        }), 404
